...
if [ "$#" -lt 7 ]; then
    echo "sh multi_attack_chat_response.sh base_dir last_epoch max_tokens infer_freq temperature top_p backbone [target_checkpoint] [provider local|api] [api_provider hf|gemini] [api_model_id]"
    exit;
fi
